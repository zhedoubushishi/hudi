...
private[hudi] object HoodieSparkSqlWriter {
  ...
  : (Boolean, common.util.Option[String], common.util.Option[String],
    ...
    if (mode == SaveMode.Ignore && tableExists) {
      ...
      if (!tableExists) {
        val archiveLogFolder = hoodieConfig.getStringOrDefault(
          HoodieTableConfig.HOODIE_ARCHIVELOG_FOLDER_PROP, "archived")
      ...
      val (writeResult, writeClient: SparkRDDWriteClient[HoodieRecordPayload[Nothing]]) =
        if (operation != WriteOperationType.DELETE) {
          ...
          val shouldCombine = hoodieConfig.getBoolean(INSERT_DROP_DUPS_OPT_KEY) || operation.equals(WriteOperationType.UPSERT);
  ...
  private def buildSyncConfig(basePath: Path, hoodieConfig: HoodieConfig): HiveSyncConfig = {
    ...
    if (syncAsDtaSourceTable) {
      ...
      hiveSyncConfig.serdeProperties = createSqlTableSerdeProperties(hoodieConfig, basePath.toString)
    }
    hiveSyncConfig
  }
