...
class DefaultSource extends RelationProvider
  ...
  override def createRelation(sqlContext: SQLContext,
    ...
    HoodieRecord.HOODIE_META_COLUMNS.asScala.toList.foreach {
      colName => df2 = df2.drop(colName)
    }

    if (parameters(OPERATION_OPT_KEY).equals(BOOTSTRAP_OPERATION_OPT_VAL)) {
      HoodieSparkSqlWriter.bootstrap(sqlContext, mode, parameters, df2)
    } else {
      HoodieSparkSqlWriter.write(sqlContext, mode, parameters, df2)
    }
    new HoodieEmptyRelation(sqlContext, df2.schema)
  }
