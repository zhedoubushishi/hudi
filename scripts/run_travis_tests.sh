...
if [ "$mode" = "unit" ]; then
  mvn clean install -Pscala-2.12 -Pspark3 -DskipTests -q
  echo "Running Unit Tests"
  mvn test -Pscala-2.12 -Pspark3 -Punit-tests -pl "$modules" -B
elif [ "$mode" = "functional" ]; then
  echo "Running Functional Tests"
  mvn test  -Pscala-2.12 -Pspark3 -Pfunctional-tests -B
  ...
  mvn verify  -Pscala-2.12 -Pspark3 -Pintegration-tests -B
else
