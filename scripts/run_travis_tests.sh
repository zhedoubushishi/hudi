...
if [ "$mode" = "unit" ]; then
  mvn clean install -DskipTests -q -Dscala-2.12 -Pspark3
  echo "Running Unit Tests"
  mvn test -Punit-tests -pl "$modules" -B -Dscala-2.12 -Pspark3
elif [ "$mode" = "functional" ]; then
  echo "Running Functional Tests"
  mvn test -Pfunctional-tests -B -Dscala-2.12 -Pspark3
  ...
  mvn verify -Pintegration-tests -B -Dscala-2.12 -Pspark3
else
