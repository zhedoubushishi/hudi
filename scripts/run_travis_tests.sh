...
if [ "$mode" = "unit" ]; then
  mvn clean install -Pspark3 -Pscala-2.12 -DskipTests -q
  echo "Running Unit Tests"
  mvn test -Pspark3 -Pscala-2.12 -Punit-tests -pl "$modules" -B
elif [ "$mode" = "functional" ]; then
  echo "Running Functional Tests"
  mvn test -Pspark3 -Pscala-2.12 -Pfunctional-tests -B
  ...
  mvn verify -Pspark3 -Pscala-2.12 -Pintegration-tests -B
else
